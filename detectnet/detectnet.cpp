...
//                      << ", h = " << b.h;
            ...
            cv::rectangle(img,Point(left,top),Point(right,bot),LabelColor[cls],3,8,0);
            std::string txt = City_Label[cls];
            cv::putText(img, txt, cv::Point(left, top), cv::FONT_HERSHEY_COMPLEX, 1, LabelColor[cls],2);
            LOG(INFO) << "  left = " << left
                      << ", right = " << right
                      << ", top = " << top
                      << ", bot = " << bot;
